...
export default function WatermarkLogo3D() {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // Calculate scroll velocity rotation (clamped to ±35deg)
      const rawTarget = deltaY * 0.4;
      targetRotateY = Math.max(-35, Math.min(35, rawTarget));
    };

    const updateAnimation = () => {
      // Smooth lerp towards target, with target decaying towards 0 (inertia)
      currentRotateY += (targetRotateY - currentRotateY) * 0.08;
      targetRotateY *= 0.92; // Inertia decay
  ...
  return (
    <div
      ref={containerRef}
      className="fixed top-1/2 left-1/2 z-0 pointer-events-none w-[80vw] max-w-[900px] opacity-[0.07] dark:opacity-[0.08] transition-opacity duration-300"
      style={{
    ...
    >
      ...
      >
        <defs>
          <linearGradient id="creuWatermarkGradient" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stopColor="rgba(180, 200, 220, 0.9)" />
            <stop offset="25%" stopColor="rgba(255, 255, 255, 0.95)" />
            <stop offset="50%" stopColor="rgba(200, 180, 190, 0.7)" />
            <stop offset="75%" stopColor="rgba(255, 255, 255, 0.85)" />
            <stop offset="100%" stopColor="rgba(160, 190, 210, 0.8)" />
          </linearGradient>
        </defs>
        <g>
          {/* Path 1: Logo Main Symbol */}
          <path
            fill="url(#creuWatermarkGradient)"
