...
export default function WatermarkLogo3D() {
  ...
  useEffect(() => {
    ...
    const updateAnimation = () => {
      ...
      // Track scroll velocity for 3D Y-rotation
      if (Math.abs(deltaY) > 0.05) {
        const rawTarget = deltaY * 0.55;
        targetRotateY = Math.max(-45, Math.min(45, rawTarget));
      }

      // Smooth lerp + inertia decay
      currentRotateY += (targetRotateY - currentRotateY) * 0.08;
      targetRotateY *= 0.90; // Velocity decay
  ...
  return (
    <div
      ...
      className="fixed top-1/2 left-1/2 pointer-events-none w-[80vw] max-w-[900px] opacity-[0.09] dark:opacity-[0.10] transition-opacity duration-300"
      style={{
        zIndex: 0,
    ...
    >
      ...
      >
        <defs>
          {/* Frosted Acrylic Glass Filter */}
          <filter id="acrylicBlur" x="-10%" y="-10%" width="120%" height="120%">
            <feGaussianBlur stdDeviation="1.2" />
          </filter>

          {/* Glass Edge Stroke Gradient */}
          <linearGradient id="acrylicGlassStroke" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stopColor="rgba(255, 255, 255, 0.55)" />
            <stop offset="50%" stopColor="rgba(195, 210, 245, 0.35)" />
            <stop offset="100%" stopColor="rgba(255, 255, 255, 0.5)" />
          </linearGradient>

          {/* Dark Mode Glass Stroke Gradient */}
          <linearGradient id="acrylicGlassStrokeDark" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stopColor="rgba(255, 255, 255, 0.65)" />
            <stop offset="50%" stopColor="rgba(196, 153, 245, 0.45)" />
            <stop offset="100%" stopColor="rgba(255, 255, 255, 0.55)" />
          </linearGradient>
        </defs>

        <g filter="url(#acrylicBlur)">
          {/* Path 1: Logo Main Symbol (Frosted Glass Fill + Crisp Stroke) */}
          <path
            fill="rgba(255, 255, 255, 0.05)"
            stroke="url(#acrylicGlassStroke)"
            strokeWidth="0.65"
