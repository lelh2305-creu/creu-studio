...
async function main() {
  ...
  if (!thumbUrl) {
    console.log('Catbox timeout/fail, trying Tmpfiles for 47pm...');
  ...
  console.log('\n--- UPLOAD RESULTS ---');
  console.log('THUMB_URL (47pm):', thumbUrl);
  console.log('EXTRA_URL:', extraUrl);

  if (!thumbUrl) {
    console.error('Failed both upload providers');
    process.exit(1);
  }
  ...
  // Update or insert extraUrl image into content if present
  if (extraUrl) {
  ...
  console.log('Updated quay-chup-noi-that-hcm.mdx with new 47pm thumbnail!');
